...
class APIClient:
    ...
    def _convert_report_to_comments(self, analysis_report: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        logger.debug(f"Converting report with {analysis_report['summary']['total_issues']} issues")
            ...
            for group in grouped_issues:
                if len(group['issues']) > 3:  # Consolidate if more than 3 similar issues
                    ...
                    for issue in group['issues']:
                        ...
                        logger.debug(f"Formatting issue: {issue.get('title', 'Unknown')}")
                        logger.debug(f"Issue data: remediation='{issue.get('remediation', 'MISSING')}', description='{issue.get('description', 'MISSING')[:50]}...'")
...
4. Ensure no regressions"""
    ...
    def _analyze_with_anthropic(self, diff: str, prompt_template: str, coderabbit_comments: List[Dict[str, Any]] = None, github_ai_prompt: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"AI response (first 500 chars): {analysis[:500]}...")

            # Parse analysis into review comments
            parsed_comments = self._parse_ai_analysis(analysis)
            logger.debug(f"Parsed {len(parsed_comments)} comments from AI response")
    ...
    def _extract_issue_from_section(self, section: str) -> Optional[Dict[str, Any]]:
        ...
        if not any(keyword in section.lower() for keyword in [
            ...
            'location:', 'issue:', 'fix:'  # Brutal prompt markers
        ]):
            return None
        
        # Check for brutal prompt format with Location/Issue/Fix structure
        location_match = re.search(r'\*\*Location\*\*:\s*(?:EXACT file path and line number as\s*)?`?([a-zA-Z0-9_/.-]+\.py):([0-9]+)`?', section)
        issue_match = re.search(r'\*\*Issue\*\*:\s*(.+?)(?:\n|$)', section)
        fix_match = re.search(r'\*\*Fix\*\*:\s*(.+?)(?:\n\n|$)', section, re.DOTALL)
        
        if location_match and issue_match:
            # Brutal prompt format detected
            file_path = location_match.group(1)
            line_number = int(location_match.group(2))
            title = issue_match.group(1).strip()
            remediation = fix_match.group(1).strip() if fix_match else "Apply the recommended fix"
            
            # Extract severity from content
            severity = "info"
            if any(word in section.lower() for word in ['critical', 'severe', 'security', 'vulnerability', 'injection', 'hardcoded', 'eval', 'exec']):
                severity = "critical"
        ...
        # Fallback to original parsing for non-brutal formats
